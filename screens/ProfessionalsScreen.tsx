...
export default function ProfessionalsScreen({ route }: any) {
  ...
  const handleViewProfile = async (professional: Professional) => {
    ...
    try {
      ...
      setShowProfile(true);
       ...
       // Silent fail or low priority alert
  ...
  // ✅ FIXED: Navigates to PatientCallHistory
  const handleViewCallHistory = () => {
      if (!user) {
          Alert.alert(i18n.t("sign_in_required"), i18n.t("please_sign_in"));
          return;
      }
      navigation.navigate('PatientCallHistory');
  };
            ...
            <ScrollView style={styles.modalBody}>
              ...
              {professional.bio && (
                <View style={{ marginBottom: Spacing.lg }}>
                  <ThemedText weight="medium" style={{ marginBottom: Spacing.sm }}>{i18n.t("about")}</ThemedText>
                  <ThemedText style={{ color: theme.textSecondary }}>{professional.bio}</ThemedText>
                </View>
              )}
  
              <View style={{ marginBottom: Spacing.lg }}>
                <ThemedText weight="medium" style={{ marginBottom: Spacing.sm }}>{i18n.t("details")}</ThemedText>
              ...
              {reviews.length > 0 && (
                <View>
                  <ThemedText weight="medium" style={{ marginBottom: Spacing.sm }}>{i18n.t("recent_reviews")}</ThemedText>
                  {reviews.slice(0, 3).map((review) => (
                    <View key={review.id} style={[styles.reviewCard, { backgroundColor: theme.backgroundSecondary }]}>
                      <View style={{ flexDirection: "row", alignItems: "center", marginBottom: Spacing.xs }}>
                        {[...Array(5)].map((_, i) => (
                          <Feather key={i} name="star" size={12} color={i < review.rating ? "#fbbf24" : theme.border} style={{ marginRight: 2 }} />
                        ))}
                      </View>
                      <ThemedText type="caption" style={{ color: theme.textSecondary }}>"{review.comment}"</ThemedText>
                      <ThemedText type="caption" style={{ color: theme.textSecondary, marginTop: Spacing.xs }}>- {review.patientName}</ThemedText>
                    </View>
                  ))}
                </View>
              )}
...
const BookingModal = ({ professional, onClose, onConfirm }: {
  ...
  useEffect(() => {
    if (selectedDate && selectedDate.length >= 10) loadAvailableSlots();
  }, [selectedDate]);
        ...
        <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={{ flex: 1, justifyContent: "flex-end" }}>
          <View style={[styles.modalContent, { backgroundColor: theme.cardBackground }]}>
            <View style={[styles.modalHeader, { backgroundColor: theme.primary }]}>
              ...
              <Pressable onPress={onClose} disabled={submitting}><Feather name="x" size={24} color="#fff" /></Pressable>
            </View>
            <ScrollView style={styles.modalBody}>
              <View style={[styles.professionalInfo, { backgroundColor: theme.backgroundSecondary }]}>
                ...
                <View>
                  ...
                  <ThemedText type="caption" style={{ color: theme.textSecondary }}>{professional.specialization}</ThemedText>
                  <ThemedText weight="medium" style={{ color: theme.primary, marginTop: Spacing.xs }}>₦{professional.consultationFee?.toLocaleString()}</ThemedText>
                </View>
              </View>
              ...
              <View style={styles.section}>
                ...
                {loadingSlots ? (
                  <ThemedText type="caption" style={{ color: theme.textSecondary }}>{i18n.t("loading")}...</ThemedText>
                ) : availableSlots.length === 0 ? (
                  <ThemedText type="caption" style={{ color: theme.textSecondary }}>{selectedDate ? i18n.t("no_slots_available") : i18n.t("select_date_first")}</ThemedText>
                ) : (
                  <View style={styles.timeGrid}>
                    {availableSlots.map(time => (
                      <Pressable
                        key={time}
                        style={[styles.timeSlot, { backgroundColor: selectedTime === time ? theme.primary + "20" : theme.backgroundSecondary, borderColor: selectedTime === time ? theme.primary : theme.border }]}
                        onPress={() => setSelectedTime(time)}
                        disabled={submitting}
                      >
                        <ThemedText weight="medium" style={{ color: selectedTime === time ? theme.primary : theme.text }}>{time}</ThemedText>
                      </Pressable>
                    ))}
                  </View>
                )}
              ...
              <View style={styles.section}>
                <ThemedText weight="medium">{i18n.t("consultation_reason")}</ThemedText>
                <TextInput
            ...
            <View style={[styles.modalFooter, { borderTopColor: theme.border }]}>
              <PrimaryButton title={i18n.t("cancel")} onPress={onClose} variant="outlined" style={{ flex: 1 }} disabled={submitting} />
              <PrimaryButton title={submitting ? i18n.t("submitting") : i18n.t("submit_request")} onPress={handleConfirm} disabled={!selectedDate || !selectedTime || submitting} style={{ flex: 1, marginLeft: Spacing.sm }} />
            </View>
          </View>
        </KeyboardAvoidingView>
      </View>
          ...
          <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={{ flex: 1, justifyContent: "flex-end" }}>
            <View style={[styles.modalContent, { backgroundColor: theme.cardBackground }]}>
              ...
              <ScrollView style={styles.modalBody}>
                <View style={{ alignItems: 'center', marginBottom: Spacing.lg }}>
                    <Image source={{ uri: professional.imageUrl || PLACEHOLDER_IMAGE }} style={styles.profileImageLarge} />
                    <ThemedText type="h2" style={{ marginTop: Spacing.md, color: theme.error }}>{i18n.t("immediate_attention")}</ThemedText>
              ...
              <View style={[styles.modalFooter, { borderTopColor: theme.border }]}>
                <PrimaryButton title={i18n.t("cancel")} onPress={onClose} variant="outlined" style={{ flex: 1 }} disabled={submitting} />
                <PrimaryButton title={submitting ? i18n.t("requesting") : i18n.t("start_emergency")} onPress={handleConfirm} disabled={submitting} style={{ flex: 1, marginLeft: Spacing.sm, backgroundColor: theme.error }} />
              </View>
            </View>
          </KeyboardAvoidingView>
        </View>
...
const styles = StyleSheet.create({
  ...
  callHistoryIcon: { padding: Spacing.sm, borderRadius: BorderRadius.full },
  ...
  // Card Styles
  professionalCard: { borderRadius: BorderRadius.lg, borderWidth: 1, marginBottom: Spacing.lg, overflow: "hidden", elevation: 2, shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.08, shadowRadius: 4 },
  topSection: { flexDirection: "row", padding: Spacing.lg, gap: Spacing.md },
  imageWrapper: { position: "relative", width: 120, height: 120 },
  professionalImage: { width: "100%", height: "100%", borderRadius: BorderRadius.lg, resizeMode: "cover" },
  imageBadges: { position: "absolute", top: 6, right: 6, gap: 4 },
  ...
  verifiedBadge: { width: 28, height: 28, borderRadius: 14, justifyContent: "center", alignItems: "center" },
  profInfoContainer: { flex: 1, justifyContent: "flex-start" },
  nameRow: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginBottom: 4 },
  profName: { fontSize: 18, flex: 1 },
  specialization: { fontSize: 14, marginBottom: 6 },
  ratingRow: { flexDirection: "row", alignItems: "center", gap: 4, marginTop: 4 },
  ratingText: { fontSize: 14 },
  feeContainer: { padding: Spacing.sm, borderRadius: BorderRadius.sm, alignItems: "center" },
  
  // Bottom Section Styles
  bottomSection: { padding: Spacing.lg, paddingTop: 0, gap: Spacing.md },
  statsRow: { flexDirection: "row", gap: Spacing.sm },
  statBox: { flex: 1, padding: Spacing.md, borderRadius: BorderRadius.md, alignItems: "center", justifyContent: "center", minHeight: 50 },
  actionRow: { flexDirection: "row", gap: Spacing.sm },
  actionButton: { flexDirection: "row", alignItems: "center", justifyContent: "center", paddingVertical: Spacing.md, paddingHorizontal: Spacing.sm, borderRadius: BorderRadius.md },
  emergencyButton: { flexDirection: "row", alignItems: "center", justifyContent: "center", paddingVertical: Spacing.md, borderRadius: BorderRadius.md },
  ...
  textArea: { borderWidth: 1, borderRadius: BorderRadius.md, padding: Spacing.md, marginTop: Spacing.sm, minHeight: 100, textAlignVertical: "top" },
  ...
  detailRow: { flexDirection: "row", justifyContent: "space-between", padding: Spacing.md, borderRadius: BorderRadius.md, marginBottom: Spacing.xs },
  reviewCard: { padding: Spacing.md, borderRadius: BorderRadius.md, marginBottom: Spacing.sm },
  infoBox: { flexDirection: 'row', padding: Spacing.lg, borderRadius: BorderRadius.md, borderWidth: 1, marginTop: Spacing.xl, alignItems: 'center' },
});